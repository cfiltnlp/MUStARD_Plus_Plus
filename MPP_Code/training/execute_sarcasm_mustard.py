...
class Tee(object):
    """
        This class is used for printing the logs in the file as well as on the Terminal
    """
...
# Loading data
path = "MPP_Code/data/"
xx = pd.read_csv("/home/development/apoorvan/AN_MTP/SER/data/" +
                 'MUStARD-Final.csv')
dd = []
for group in xx.groupby('KEY'):
    dd.append(group[1].values[-1])
mustard_input = pd.DataFrame(dd, columns=xx.columns)
print(mustard_input.columns)

temp = open(path+'extracted_features/mustard_all_features.pickle', 'rb')
...
class ContentDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        index = self.mapping.loc[idx, 'KEY']
        data = self.dataset[index]
        label = int(self.mapping.loc[idx, 'SARCASM'])
...
def evaluation(loader, mod, call, report=False, flag=False):
    ...
    with torch.no_grad():
        ...
        # plot(true, np.argmax(pred, axis=1), '5FOLD_MULTITASK')
...
def training(mod, criterion, optimizer, call, train_loader, valid_loader, fold, e=500, patience=5, report=False,save=False):
    """Args:
            ...
            report:
                If True then the classification report for the validation set is printed, it is given as a input to evaluation class
            save:
...
#                     ), '/home/development/apoorvan/AN_MTP/SER/saved_models/rough_saved_models/V_T_A_average_5FOLD_FOLD'+str(fold)+'_'+str(e)+'.pth')
                print(
...
""" File to store log"""
f = open('MPP_Code/log/sarcasm_mutard/' +
         filename+'.txt', 'w')
...
for dropout in [0.2, 0.3, 0.4, 0.5]:
    for lr in [0.001, 0.0001, 0.00001]:
        for batch_size in [128, 64]:
            for shared_embedding_size, projection_embedding_size in zip([2048, 1024], [1024, 512]):
                ...
                for fold in range(5):
                    ...
                    """
                    train = pd.read_csv(
                        'MPP_Code/data/split_mustard/train_' + str(fold)+'.csv')
                    valid = pd.read_csv(
                        'MPP_Code/data/split_mustard/test_' + str(fold)+'.csv')
                    seed()
                    ...
                    indexes.extend(valid['KEY'].tolist())
                ...
                stats.to_csv('MPP_Code/stats/sarcasm_mutard/' +
                ...
                results.to_csv('MPP_Code/predictions/sarcasm_mutard/' + args.mode+'/' +
