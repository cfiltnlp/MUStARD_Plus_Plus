...
The classification scores and corresponding hyperparamters get stored in stats folder.
The classification reports from sklearn get stored in charts folder.
...
class Tee(object):
    """
        ...
        Object of this class is used for printing the log in file as well as oin Terminal
    """
    ...
    def write(self, obj):
        for f in self.files:
            f.write(obj)
            f.flush()  # If we want the output to be visible immediately

    def flush(self):
        for f in self.files:
            f.flush()

seed_dyn = 42 #Ignore seed_dyn if you are trying to randomize
abl = "6" #To perform ablation in audio

def seed():
    """ This method is used for seeding the code and different points"""
    np.random.seed(seed_dyn)
    random.seed(seed_dyn)
    torch.manual_seed(seed_dyn)
    torch.cuda.manual_seed(seed_dyn)
...
'''
path = "MPP_Code/data/"
mustard_input = pd.read_csv(path+'final_datasets/augmented_sarcastic_utterances.csv')

# All features audio, Video and text saved in dictionary format
# temp = open(path+'extracted_features/an_merged/all_features_aud_text.pickle', 'rb')
# temp = open(path+'extracted_features/an_merged/ablation_'+abl+'.pickle', 'rb')
# temp = open(path+'extracted_features/all_features.pickle', 'rb')
# temp = open(path+'extracted_features/all_features_with_t5.pickle', 'rb')
...
class ContentDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label = int(self.mapping.loc[idx, 'Emo_I'])-1
...
def evaluation(loader, mod, call, report=False, flag=False):
    ...
    with torch.no_grad():
        ...
        true = []
        total_loss = []
        criterion = nn.CrossEntropyLoss() #Loss function for calculating validation loss
        criterion.to(device)
        seed()
        for batch in loader:
            ...
            del batch
            output = torch.softmax(eval(call), dim=1) # softmax is claculated explicitly
            # call is the command to be executed, since we have different combination of input modality, this decides the input by default
        ...
        if flag:
            # for directly predicting the emotion class label
            return true, np.argmax(pred, axis=1)
        if report:
            # for printing the classification report on validation set.
...
def training(mod, criterion, optimizer, call, train_loader, valid_loader, fold, e=500, patience=5, report=False,save=False):
    """Args:
            mod :
    ...
    if save:
        best_model.to('cpu')
        torch.save(best_model.state_dict(), 'MPP_Code/saved_models/emotion/implicit' +
...
def get_model_and_parameters(args):
    ...
    """
    # Here we are sorting VTA in descending order, in order to vae consistency in the model
    ...
    if len(input_modes) == 3:
        ...
        parameters['input_embedding_C'] = audio_embedding_size #319

    elif len(input_modes) == 2:
        MODEL_NAME += 'Dual_'
        parameters['input_embedding_A'] = 2048 if input_modes[0] == 'V' else 1024
        parameters['input_embedding_B'] = audio_embedding_size if input_modes[1] == 'A' else 1024
    else:
        MODEL_NAME += 'Single_'
        parameters['input_embedding_A'] = 2048 if input_modes == 'V' else 1024 if input_modes == 'T' else audio_embedding_size
...
""" File to store log"""
f = open('MPP_Code/log/aug_complete_implicit/an_aug_'+
         filename+'.txt', 'a+')
""" File to store charts of clf report"""
c = open('MPP_Code/charts/aug_complete_implicit/an_aug_'+
         filename+'.txt', 'a+')
...
print(MODEL_NAME.split('.')[1]) # printing MODEL NAME in both places

sys.stdout = original

""" since we are loading speaker name from dict 
    we are sorting them in ascending order to remove randomness and make code reproducable
"""
    ...
    for lr in [0.001, 0.0001]:
        for batch_size in [128, 64]:
            for shared_embedding_size, projection_embedding_size in zip([2048, 1024], [1024, 256]):
                ...
                # Following lists are used for calculating overall results
                pred_all = []
                true_all = []
                indexes = []
                types = []
                ...
                for fold in range(5):
                    ...
                    """
                    train = pd.read_csv(
                        'MPP_Code/data/splits_aug_mustard++/train_' + str(fold)+'.csv')
                    valid = pd.read_csv(
                        'MPP_Code/data/splits_aug_mustard++/test_' + str(fold)+'.csv')
                    seed()
                    ...
                    seed()
                    mod = eval(MODEL_NAME)(**parameters) # MODEL intitalization
                    seed()
                ...
                stats.to_csv('MPP_Code/stats/aug_complete_implicit/an_aug_'+
                ...
                results.to_csv('MPP_Code/predictions/aug_complete_implicit/'+ args.mode+'/an_aug_' +
                               filename+'_'+str(int(dropout*10))+'_'+str(batch_size)+'_'+str(shared_embedding_size)+'_'+str(len(str(lr)))+'_'+'.csv', index=False)

                """ Following code can be used to save Sarcasm type wise analysis (will be saved in log file)"""

                sys.stdout = Tee(sys.stdout, f)
                for ty in ['PRO', 'LIK', 'ILL', 'EMB','NONE']:
